...
Each column in `gut_taxonomic_table.csv` represent some taxonomic group of microbes given by a list of hierarchical categories. For example column named `Bacteria.Bacillota.Bacilli.Lactobacillales.Lactobacillaceae.Lentilactobacillus` represents the abundance of the genus Lentilactobacillus, which belongs to the family Lactobacillaceae, order Lactobacillales, class Bacilli, phylum Bacillota, and domain Bacteria. Each cell in that column contains the number of DNA sequencing reads that could be assigned to that taxonomic group in a given sample, which is a proxy for the relative abundance of that group in the sample. 
